...
import SimpleSearchFilterUrlComponentDoc from './_components/simple-search-filter-url-example.mdx';
...
import {Aspect, Center, Image, Message} from '@marigold/components';
import SimpleSearchFilterUrlComponent from './_components/SimpleSearchFilterUrlComponent';
...
<Content>
    ...
    Finally we stripped away all the prop-drilling stuff and we know exactly where the state is used.
    ...
    * Performance: If not used carefully, it can lead to unnecessary re-renders of components.

    **Use Cases:**

    * User authentication status.
    * theme settings (dark/light mode).
    * User preferences that need to be accessible globally.

    ### URL state
    URL State is used to manage state that should be reflected in the URL, which can be bookmarked or shared. This is particularly useful for representing the state of the application in a way that is persistent and shareable.
    ...
    Let's see how we can improve our `SimpleSearchFilterComponent` from above when using URL state.

    <Preview>
        <SimpleSearchFilterUrlComponent />
    </Preview>

    When you type something in the search you will see how the URL param is updated. In our case we use `tanstack-router`. They provide some useful hooks for working with URL params.
...
<Content>
    ...
    **Security**: Avoid storing sensitive information in the URL, as it is not secure and can be easily accessed by others.
    ...
    Another approach in state management is using a `server cache`. Server Cache refers to storing frequently accessed data on the server side to improve performance and reduce the load on the backend database or other persistent storage.
    ...
    **Requested data already cached:** When the client sends a request, the server first checks the cache. If the data is present, the response is returned immediately.

    **Requested data isn't cached:** When the client sends a request, the server first checks the cache. The data isn't available yet, the server fetches the data from the primary storage, stores it in the cache for future requests, and then returns it to the client.
    ...
    * Open the network tab in the developer tools. Open another tab and go back to the previous. You will see that the data request is executed again. The same happens when you turn the network off/on.
