...
import { Breakout, Center, Message } from '@marigold/components';
...
<Content>
    ...
    * **Security**: Avoid storing sensitive information in the URL, as it is not secure and can be easily accessed by others.
    ...
    <Message messageTitle='Note'>
        Even it is called server cache it doesn't belongs to the server itself! It's on the client side and stored in-memory.
    </Message>
</Content>
