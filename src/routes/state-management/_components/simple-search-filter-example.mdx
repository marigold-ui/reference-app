...
<Block>
    <CH.Scrollycoding>
        ```tsx SimpleSearchFilterComponent.tsx focus=11:12,23
        // from ./SimpleSearchFilterComponent.tsx
        ````

        ### Storing an initial value

        Here we hold two local state. `searchQuery` is for saving the current input value. `filteredList` holds all filtered movies based on the searchQuery.

        ---

        ```tsx SimpleSearchFilterComponent.tsx focus=52:57
        // from ./SimpleSearchFilterComponent.tsx
        ````
        ...
        ```tsx SimpleSearchFilterComponent.tsx focus=27:34,47:51
        // from ./SimpleSearchFilterComponent.tsx
        ````
